  ...
  <div class="bg-white flex flex-col gap-x-3 rounded-md shadow-sm">
    <div class="p-3">
      ...
      <div class="flex items-center justify-center mt-2 gap-x-1">
        ...
        <button class="bookmark-btn group" :class="{ 'bookmark-item-active': alreadyBookmarked }" @click="bookmarkItem">
          <svg
    ...
    <!-- Burada categorileri mountedında categori tablosuna sorgu atıp ordan title alıp kullanırsak tüm sistemi patlatırız BU YAPILMAMASI GEREKEN BİR ŞEYDİR -->
    <!-- Bunun için typicode json-serverın sunduğu _expand özelliği kullanılabilir postlara eklenir bu özellik-->
    <!-- Ayrıca burada artık item.category.name diyerek de direk gösterebiliriz ama gelmediği durumlarda temamız patlamasın diye bunu computed içerisinde kontrol etmek daha mantıklı olur. -->
...
export default {
  ...
  computed: {
    ...
    // like butonundaki :class tarafında kullanılarak likelanmış buttonların rengini değiştirir
    alreadyLiked() {
